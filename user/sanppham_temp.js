...
function getMenuBanhFromLocalStorage() {
  ...
  const MenuBanh = types.map(type => {
    const typeName = type.name;
    const cakes = products
      .filter(product => product.type === typeName)
      .map(product => {
        // Tìm giá tương ứng
        const priceInfo = prices.find(p => p.name === product.name) || { sale: 0 };
        return {
          name: product.name,
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    card.innerHTML = `
      ...
      <div class="product-info">
        ...
        <button class="add-to-cart" onclick="themVaoGioHang(${banh.name})>
          <i class='bx bx-cart-add'></i> Thêm vào giỏ
        </button>
      </div>
    `;

    // Gắn sự kiện click cho card (trừ nút Thêm vào giỏ)
    card.addEventListener("click", function (e) {
      if (e.target.closest(".add-to-cart")) return; // bỏ qua nếu click vào nút giỏ
...
function showProductDetail(data) {
  ...
  modalBody.innerHTML = `
    <div class="product-detail">
      ...
      <button class="add-to-cart" onclick="themVaoGioHang('${data.name}', ${
    data.price
  }, '${data.img}')">
